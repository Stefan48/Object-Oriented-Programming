...
final class GoodBoy extends Angel
{
    ...
    void influenceHero(Pyromancer pyromancer)
    {
        if(!pyromancer.dead)
        {
            pyromancer.extra_race_modifier += 0.5f;
            pyromancer.hp = Math.min(pyromancer.hp + 30, pyromancer.max_hp);
    ...
    void influenceHero(Knight knight)
    {
        if(!knight.dead)
        {
            knight.extra_race_modifier += 0.4f;
            knight.hp = Math.min(knight.hp + 20, knight.max_hp);
    ...
    void influenceHero(Wizard wizard)
    {
        if(!wizard.dead)
        {
            wizard.extra_race_modifier += 0.3f;
            wizard.hp = Math.min(wizard.hp + 50, wizard.max_hp);
    ...
    void influenceHero(Rogue rogue)
    {
        if(!rogue.dead)
        {
            rogue.extra_race_modifier += 0.4f;
            rogue.hp = Math.min(rogue.hp + 40, rogue.max_hp);
